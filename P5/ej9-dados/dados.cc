...
ostream& operator<<(ostream &stream, const Dados& dado) {
	stream << "d1: " << dado.d1_;
	stream << ", d2: " << dado.d2_;
	return stream;
}

istream& operator>>(istream &stream, Dados& dado) {
	int d1, d2;
	do {
		cout << "Introduce el valor del dado 1: ";
		stream >> d1;
		cout << "Introduce el valor del dado 2: ";
		stream >> d2;
		if (dado.setDado1(d1)==false or dado.setDado2(d2)==false)
			cout << "Introduccion incorrecta" << endl;
	} while (dado.setDado1(d1)==false or dado.setDado2(d2)==false);
	cout << "Introduccion correcta" << endl;
}